...
public class CreatePDF_iText_Png_Juz
{
	...
	CreatePDF_iText_Png_Juz() throws IOException
	{
		...
		for (int k = 0; k < 30; k++)
		{
			final PdfDocument pdfDocument = new PdfDocument(new PdfWriter(p + (hafs ? "quran_hafs_m" : "quran_warsh_m") + (k + 1) + ".pdf"
			...
			for (int i = juz_page[k]; i < (k == 29 ? 605 : juz_page[k + 1]); i++)
			{
